...
def getKmer(k, alphabet=None, dir=None):
  ...
  if dir is not None:
    kmerfilename = dir + "/" + kmerfilenameshape
...
def convert_shape(seqs, intensity, seqs_flank, mapper, shapeDict, out_path, name):
    ...
    fourMPRH_ = np.concatenate((MGW, ProT, Roll, HelT), axis=-1)
    # store fourMPRH in hdf5 format
    out_filename = out_path + '/%s_MPRH.hdf5' % name
    if os.path.exists(out_filename):
       os.remove(out_filename)
    outputHDF5Shape(fourMPRH, out_filename)
    out_filename = out_path + '/%s_MPRH_.hdf5' % name
    if os.path.exists(out_filename):
       os.remove(out_filename)
    outputHDF5Shape(fourMPRH_, out_filename)
    ...
    # store HelT in hdf5 format
    out_filename = out_path + '/%s_HelT.hdf5' % name
    if os.path.exists(out_filename):
       os.remove(out_filename)
    outputHDF5Shape(HelT_d_arr, out_filename)'''
    # store MGW and ProT in hdf5 format
    out_filename = out_path + '/%s_MP.hdf5' % name
    if os.path.exists(out_filename):
       os.remove(out_filename)
    twoMP = np.concatenate((MGW_arr, ProT_arr), axis=-1)
    outputHDF5Shape(twoMP, out_filename)
    # store Roll and HelT in hdf5 format
    out_filename = out_path + '/%s_RH.hdf5' % name
    if os.path.exists(out_filename):
       os.remove(out_filename)
    Roll_arr = Roll.reshape((Roll.shape[0], Roll.shape[1], 1))
    HelT_arr = HelT.reshape((HelT.shape[0], HelT.shape[1], 1))
    twoRH = np.concatenate((Roll_arr, HelT_arr), axis=-1)
    outputHDF5Shape(twoRH, out_filename)
...
def parse_args():
    ...
    parser.add_argument("-mapper", "--mapperfile", dest="mapperfile", default="", help="A file mapping each nucleotide to a vector.")
...
if __name__ == "__main__":
    ...
    if args.mapperfile == "":
        ...
        mapper = Load_mapper(args.mapperfile)
