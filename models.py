...
# build hybrid model
def DeepBindwithShapeone(shape1=None, shape2=None, params=None, penalty = 0.005):
    ...
    auxiliary_input = Input(shape=shape2, name='shape')
    Y = Dense(16, activation='relu', name='shape_dense')(auxiliary_input)
...
# build hybrid model
def DeepBindwithShapetwo(shape1=None, shape2=None, shape3=None, params=None, penalty = 0.005):
    
    main_input = Input(shape=shape1, name='sequence')
    X = Convolution1D(16, 13, activation='relu', padding='same', name='seq_conv')(main_input)
    X = GlobalMaxPooling1D()(X)
    
    auxiliary_MP = Input(shape=shape2, name='shape_MP')
    Y = Convolution1D(16, 13, activation='relu', padding='same', name='shape_conv_MP')(auxiliary_MP)
    Y = GlobalMaxPooling1D()(Y)
    
    auxiliary_RH = Input(shape=shape3, name='shape_RH')
    Z = Convolution1D(16, 13, activation='relu', padding='same', name='shape_conv_RH')(auxiliary_RH)
    Z = GlobalMaxPooling1D()(Z)
    
    concat = concatenate([X, Y, Z], axis=-1)
    output = Dense(32, activation='relu', kernel_regularizer=regularizers.l2(penalty))(concat)
    output = Dropout(params['DROPOUT'])(output)
    output = Dense(1)(output)
    
    model = Model(inputs=[main_input, auxiliary_MP, auxiliary_RH], outputs=output)
    print model.summary()
    return model
